...
class Transport:
    ...
    def sample(self, x1, x_noisy=None, noise_scale=0.0):
        """Sampling x0 & t based on shape of x1 (if needed)
          Args:
            x1 - data point; [batch, *dim]
            x_noisy - optional noisy starting point for flow matching; [batch, *dim]
            noise_scale - noise level to add to x_noisy (default: 0.0)
        """

        # Flow Matching: start from x_noisy if provided
        if x_noisy is not None and noise_scale > 0.0:
            # Start from noisy data with small perturbation
            noise = th.randn_like(x1) * noise_scale
            x0 = x_noisy + noise
        else:
            # Standard approach: start from random noise
            x0 = th.randn_like(x1)
