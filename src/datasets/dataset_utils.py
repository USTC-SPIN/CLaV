...
from src.datasets.base_datasets import EvaluationTuple, TrainingDataset
...
def make_datasets(params: TrainingParams, validation: bool = False):
    """
    Create training/validation datasets.
    Auto-detects BEV vs RIV based on train_file name.
    """
    # Check if using BEV dataset
    is_bev = 'bev' in params.train_file.lower()

    if is_bev:
        from src.datasets.bev_datasets import BEVTrainingDataset
        dataset_class = BEVTrainingDataset
        print("Using BEV dataset")
    else:
        dataset_class = TrainingDataset
        print("Using RIV dataset")
    ...
    if is_bev:
        ...
        if validation and params.val_file:
            datasets['val'] = dataset_class(params.dataset_folder, params.val_file)
...
def make_dataloaders(params: TrainingParams, validation: bool = False) -> Dict[str, DataLoader]:
    ...
    Auto-detects BEV vs RIV and uses appropriate collate function.
    ...
    if is_bev:
        from src.datasets.bev_dataset_utils import make_bev_collate_fn
        collate_fn_maker = make_bev_collate_fn
        print("Using simplified BEV collate function (no ICP)")
    else:
        collate_fn_maker = make_collate_fn
        print("Using RIV collate function (with ICP)")
    ...
    train_collate_fn = collate_fn_maker(datasets['train'], params)
    ...
    if validation and 'val' in datasets:
        ...
        val_collate_fn = collate_fn_maker(datasets['val'], params)
