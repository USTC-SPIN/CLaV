#!/bin/bash
# Boreas数据集Stage2训练启动脚本
# Created: 2025-11-07 21:48
# Purpose: 训练Boreas数据集的Stage2扩散去噪模型（Flow Matching）
...
# 训练配置文件
CONFIG="configs/boreas/stage2.yaml"
BASE_CONFIG="configs/boreas/base_config.yaml"

# 检查数据文件是否存在
echo "========================================"
echo "Boreas Stage2 Flow Matching 训练准备"
echo "========================================"
echo ""
echo "检查数据文件..."
DATA_FILES=(
    "data/boreas_bev_denoising_pairs_spatial.pickle"
)
...
if [ "$ALL_EXIST" = false ]; then
    ...
    echo "错误: 部分数据文件不存在"
    echo "请先运行数据生成脚本:"
    echo "  python data_pre/boreas/generate_boreas_bev_denoising_pairs_spatial.py"
    exit 1
fi
...
# 显示训练信息
echo "========================================"
echo "Boreas Stage2 训练信息"
echo "========================================"
python3 -c "
import pickle
data = pickle.load(open('$PROJECT_ROOT/data/boreas_bev_denoising_pairs_spatial.pickle', 'rb'))
...
print(f'')
print(f'天气条件: Snow, Rain')
print(f'清洁参考: Clear')
print(f'数据来源: Boreas真实采集数据')
print(f'配对方法: GPS最近邻匹配（<5m）')
"
echo "========================================"
echo ""
...
LOG_FILE="$LOG_DIR/boreas_stage2_train_${TIMESTAMP}.log"
...
# 保存PID到文件，方便后续管理
PID_FILE="$LOG_DIR/boreas_stage2_train.pid"
