#!/bin/bash
# KITTI数据集Stage3训练启动脚本
# Created: 2025-11-08
# Purpose: 训练KITTI数据集的Stage3描述符头（跨天气地理定位）
...
# 训练配置文件
CONFIG="configs/kitti/kitti_stage3_training.yaml"
BASE_CONFIG="configs/base_config.yaml"

# 检查数据文件是否存在
echo "========================================"
echo "KITTI Stage3 描述符训练准备"
echo "========================================"
echo ""
echo "检查数据文件..."
...
LOG_FILE="$LOG_DIR/kitti_stage3_train_${TIMESTAMP}.log"
...
# master_port: 29503（避免与Stage2的29502冲突）
echo "正在启动训练进程..."
if [ $# -gt 0 ]; then
...
# 保存PID到文件，方便后续管理
PID_FILE="$LOG_DIR/kitti_stage3_train.pid"
