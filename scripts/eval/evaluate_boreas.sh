...
# Boreas数据集模型评估脚本
# Created: 2025-11-08
# Purpose: 评估训练好的Boreas模型（跨天气地理定位性能）
...
# 默认参数
CHECKPOINT="/data/users/cxw/pro/clav/results/boreas_descriptor_training_v3_20251114_0432/epoc10/last.pt" #boreas_descriptor_training_optimized_20251112_1717
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --config PATH        配置文件路径（默认: configs/boreas/boreas_stage3_training.yaml）"
            echo "  --device DEVICE      设备（默认: cuda）"
            echo "  --batch_size N       批量大小（默认: 16）"
            echo "  --gpu N              GPU ID（默认: 0）"
            echo "  --help               显示此帮助信息"
            echo ""
            echo "示例:"
            echo "  bash $0 --checkpoint results/boreas_descriptor_training_*/best.pt"
            echo "  bash $0 --checkpoint results/boreas_descriptor_training_*/best.pt --gpu 1"
            exit 0
            ;;
        *)
            echo "未知选项: $1"
...
if [ -z "$CHECKPOINT" ]; then
    ...
    LATEST_DIR=$(ls -td results/boreas_descriptor_training_* 2>/dev/null | head -1)
    ...
    if [ -n "$LATEST_DIR" ] && [ -f "$LATEST_DIR/best.pt" ]; then
        ...
        echo "  bash $0 --checkpoint results/boreas_descriptor_training_<timestamp>/best.pt"
        echo ""
        echo "或确保已完成Stage3训练:"
