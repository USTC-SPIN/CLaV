...
# nclt数据集模型评估脚本
# Created: 2025-11-08
# Purpose: 评估训练好的nclt模型（跨天气地理定位性能）
...
# 默认参数
CHECKPOINT=""
CONFIG="configs/nclt/nclt_stage3_training_v2.yaml"
DEVICE="cuda"
BATCH_SIZE=64
GPU_ID=5
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --config PATH        配置文件路径（默认: configs/nclt/nclt_stage3_training.yaml）"
            ...
            echo ""
            echo "示例:"
            echo "  bash $0 --checkpoint results/nclt_descriptor_training_*/best.pt"
            echo "  bash $0 --checkpoint results/nclt_descriptor_training_*/best.pt --gpu 1"
            exit 0
            ;;
        *)
            echo "未知选项: $1"
...
if [ -z "$CHECKPOINT" ]; then
    ...
    LATEST_DIR=$(ls -td results/nclt_descriptor_training_* 2>/dev/null | head -1)
    ...
    if [ -n "$LATEST_DIR" ] && [ -f "$LATEST_DIR/best.pt" ]; then
        ...
        echo "  bash $0 --checkpoint results/nclt_descriptor_training_<timestamp>/best.pt"
        echo ""
        echo "或确保已完成Stage3训练:"
        echo "  bash scripts/train_nclt_stage3.sh"
        exit 1
    fi
fi
...
if [ "$EVAL_FILES_MISSING" = true ]; then
    ...
    echo "错误: 部分评估数据文件不存在"
    echo "请先运行数据生成脚本:"
    echo "  bash data_pre/nclt/run_all_nclt_data_generation.sh"
    exit 1
fi
